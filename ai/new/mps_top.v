...
05/30/23
	20-ID Test
	N_OF_PLC_VALID 18
	`define	AIE_USR_FULL_MASK_SETTING    64'h000000000FFFFFFF - need to change if you added a new IDs
...
`define	FIRMWARE_VER            32'h05302023  //20 ID
...
//05/30/23 update 
`define	AIE_USR_FULL_MASK_SETTING    64'h000000000FFFFFFF
...
`define	N_OF_PLC_VALID               18  //16  //5/30/23 +1 20-ID
...
`define	IN_OFF		16
    ...
    always @ (posedge clk ) begin
        if(reset == 1'b1 || GlobalLatchClearingReset == 1'b1) begin	//clear
            ...
            // ID07
            CellPLC_IdBeamDump_StatusReg[21] <= xy_pos_Reg[`C7_CDATA_BASE][`IN_OFF+5] && xy_pos_Reg[`C7_CDATA_BASE][`IN_OFF+4];
            CellPLC_IdBeamDump_StatusReg[22] <= xy_pos_Reg[`C7_CDATA_BASE][`IN_OFF+5] && xy_pos_Reg[`C7_CDATA_BASE][`IN_OFF+4];
            //9/3/2022
            CellPLC_IdBeamDump_StatusReg[23] <= xy_pos_Reg[`C27_CDATA_BASE][`IN_OFF+5] && xy_pos_Reg[`C27_CDATA_BASE][`IN_OFF+4];
            CellPLC_IdBeamDump_StatusReg[24] <= xy_pos_Reg[`C20_CDATA_BASE][`IN_OFF+5] && xy_pos_Reg[`C20_CDATA_BASE][`IN_OFF+4];
            //
...
`define	BPM_Y_OFF	180
	...
	//
	assign  Cell_PLC_WD_Status[17] = CellPLCWdStatus[23];   //#20-ID 5/30/23
...
`ifdef	__AI__CHIPSCOPE_1__
    ...
    assign trigger0[139]        = PosXyDataValid;
