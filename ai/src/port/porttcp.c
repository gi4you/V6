...
#define MB_TCP_BUF_SIZE     	(8192+7)	//( 256 + 7 ) /* Must hold a complete Modbus TCP frame. */
...
/* User variable for host information */
long	TcpErrorCountVal=0;
long	TcpConnectionCountVal=0;
...
xMBTCPPortInit( USHORT usTCPPort )
{
    ...
    if( ( pxPCBListenNew = pxPCBListenOld = tcp_new(  ) ) == NULL )
    {
        ...
        vMBPortLog( MB_LOG_DEBUG, "MBTCP-ACCEPT", "Can't create TCP socket.\r\n" );
        bOkay = FALSE;
    }
...
prvvMBPortReleaseClient( struct tcp_pcb *pxPCB )
{
    if( pxPCB != NULL )
    {
        ...
        {
#ifdef MB_TCP_DEBUG
            vMBPortLog( MB_LOG_DEBUG, "MBTCP-CLOSE", "Closed [%d] times connection to %d.%d.%d.%d.\r\n",
            			TcpConnectionCountVal++,
...
prvvMBTCPPortError( void *pvArg, err_t xErr )
{
    ...
    {
#ifdef MB_TCP_DEBUG
        vMBPortLog( MB_LOG_DEBUG, "MBTCP-ERROR", "Error with client connection! Droping it. err %d times\r\n", TcpErrorCountVal++ );
#endif        
        prvvMBPortReleaseClient( pxPCB );
    }
}
...
prvxMBTCPPortReceive( void *pvArg, struct tcp_pcb *pxPCB, struct pbuf *p, err_t xErr )
{
    ...
    else
    {
        ...
        if( usTCPBufPos >= MB_TCP_FUNC )
        {
			...
			//xil_printf("usTCPBufPos=%d usLength=%d\r\n", usTCPBufPos, usLength);	//10 : 12, 6
...
xMBTCPPortSendResponse( const UCHAR * pucMBTCPFrame, USHORT usTCPLength )
{
    ...
    if( pxPCBClient )
    {
		...
		//xil_printf("usTCPLength=%d\r\n", usTCPLength);
        //assert( tcp_sndbuf( pxPCBClient ) >= usTCPLength );
        if( tcp_sndbuf( pxPCBClient ) < usTCPLength) {
        	prvvMBPortReleaseClient( pxPCBClient );
        	return bFrameSent;
        }
		
        if( tcp_write( pxPCBClient, pucMBTCPFrame, ( u16_t ) usTCPLength, NETCONN_COPY ) == ERR_OK )	//NETCONN_COPY=1
        {
...
/*
 *	large size multiple transmitte
 */
BOOL
xMBTCPPortSendResponse( const UCHAR * pucMBTCPFrame, USHORT usTCPLength )
{
    ...
    block_num = (int)(usTCPLength / SEND_BUF_SIZE);		
    size = (int)(block_num * SEND_BUF_SIZE);
    rsize =  usTCPLength - size;
    
    //xil_printf("usTCPLength=%d, block=%d,  size=%d,  r_size=%d\r\n\r\n", usTCPLength, block_num, size, rsize);
    ...
    if( pxPCBClient )
    {
		...
		{
			assert( tcp_sndbuf( pxPCBClient ) >= SEND_BUF_SIZE );	//Returns the number of bytes of space available in the output queue. 		
			if( tcp_write( pxPCBClient, &pucMBTCPFrame[total_byte_send], ( u16_t ) SEND_BUF_SIZE, NETCONN_COPY ) == ERR_OK )
			{
				/* Make sure data gets sent immediately. */
				( void )tcp_output( pxPCBClient );
				bFrameSent = TRUE;
		...
		//
		assert( tcp_sndbuf( pxPCBClient ) >= rsize );		
		if( tcp_write( pxPCBClient, &pucMBTCPFrame[total_byte_send], ( u16_t ) rsize, NETCONN_COPY ) == ERR_OK )
		{
			/* Make sure data gets sent immediately. */
			( void )tcp_output( pxPCBClient );
			bFrameSent = TRUE;
